...
from sklearn.metrics import mean_squared_error
rmse = np.sqrt(mean_squared_error(test['Bookings'], y_hat_ar['ar_forecast'][test.index.min():])).round(2)
mape = np.round(
np.mean(np.abs(test['Bookings'] - y_hat_ar['ar_forecast'][test.index.min():]) / test['Bookings']) * 100, 2)
